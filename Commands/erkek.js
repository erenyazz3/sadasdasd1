...
if(!member) return message.channel.send(embed.setDescription(`Bir kullanıcı belirtmelisin. \`.erkek [üye] [isim] [yaş]\``).setColor(ayarlar.beyaz)).then(message.react(ayarlar.çarpı)).then(x=>x.delete({timeout:3500}))
let isim = args[1]
let yaş = args[2]
if(!isim || !yaş) return message.channel.send(embed.setDescription(`Bir isim ve yaş girmelisin. \`.erkek [üye] [isim] [yaş]\``).setColor(ayarlar.beyaz)).then(message.react(ayarlar.çarpı)).then(x=>x.delete({timeout:3500}))
...
let e1 = "788679998127079444"
if(member.roles.cache.has('788679995177566258')){
    await member.setNickname(`${ayarlar.tag} ${isim} | ${yaş}`)
    await qdb.push(`${message.guild.id}_data${member.id}`, `• ${isim} | ${yaş} (<@&${e1}>)`)
} else {
    await member.setNickname(`• ${isim} | ${yaş}`)
    await qdb.push(`${message.guild.id}_data${member.id}`, `• ${isim} | ${yaş} (<@&${e1}>)`)
}
await member.roles.add('788679998127079444')
await member.roles.add('788680001955430470')
await member.roles.add('788679999691554846')
...
log.send(embed.setDescription(`${member} (\`${member.id}\`) Kullanıcısı ${message.author} (\`${message.author.id}\`) tarafından \`${tarih}\` tarihinde <@&${e1}> rolleri verilerek kayıt edildi.`).setColor(ayarlar.yeşil))
};
module.exports.configuration = {
    name: "erkek",
    aliases: ["e", "bay"],
    usage: "erkek [üye] [isim] [yaş]",
    description: "Erkek kayıt komutudur.",
    permLevel: 0
};