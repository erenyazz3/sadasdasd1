...
client.on("message", (message) => {
      if (message.author.bot ||!message.content.startsWith(ayarlar.botPrefix) || !message.channel || message.channel.type == "dm") return;
      if (waitLimit[message.author.id] && (Date.now() - waitLimit[message.author.id]) / (1000) <= ayarlar.waitLimit) return;
      let args = message.content
        .substring(ayarlar.botPrefix.length)
        .split(" ");
      let command = args[0];
